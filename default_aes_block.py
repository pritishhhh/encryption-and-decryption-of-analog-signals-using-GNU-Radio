...
class blk(gr.sync_block):
    """
    AES ENCRYPT + DECRYPT IN ONE BLOCK
    ────────────────────────────────────
    ...
    def __init__(self):
        ...
        with open(f"{base}/aes_key.bin", "wb") as f:
            f.write(self.key)
    ...
    def work(self, input_items, output_items):
        for i in range(len(input_items[0])):
            ...
            try:
                # ── Step 3: ENCRYPT ──
                cipher_enc = AES.new(self.key, AES.MODE_CBC, iv=iv)
                ...
                if len(recovered) == 256:
                    ...
                    self.success += 1
                    print(f"[AES] Chunk {self.chunk_count:04d} | "
                ...
                print(f"[AES] Chunk {self.chunk_count:04d} | Error: {e}")
                output_items[0][i] = vector
